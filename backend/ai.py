...
def build_data_context(is_admin=False):
    ...
    ctx += "\n- water_pressure: 末端水压设备. 关键字段: project_name, device_name(设备名称), install_loc(安装位置), "
    ctx += "pressure(实时水压), pressure_fault(压力故障,值仅【失压】/【超压】/【0】, 离线设备的值为'0'), "
    ctx += "online_status(在线状态,值【在线】/【离线】), comm_delay(通讯时差,单位天), report_time(上报时间), "
    ctx += "net_signal(网络信号), probe_battery(探测器电量), module_battery(模块电量)"
    ctx += "\n  ⚠ 重要: 快照中的「水压离线」计数按 comm_delay >= 2(天) 判定, 与 calc 口径一致. "
    ctx += "pressure_fault 只标记失压/超压, 不含【离线】. 查离线设备务必用 comm_delay >= 2 或 "
    ctx += "online_status LIKE '%离线%', 切勿用 pressure_fault 查离线(返回0行)"
    ctx += "\n  查水压离线设备明细(含位置和离线时长): SELECT device_name, install_loc, pressure, online_status, "
    ctx += "comm_delay, report_time FROM water_pressure WHERE project_name = '项目名' AND comm_delay >= 2"
    ctx += "\n  查水压失压设备: SELECT device_name, install_loc, pressure, report_time FROM water_pressure "
    ctx += "WHERE project_name = '项目名' AND pressure_fault = '失压'"
